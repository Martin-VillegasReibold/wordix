 ...
 * @return int $puntos
 */
function obtenerPuntajeWordix($palabra,$inten)  /* ****COMPLETAR***** parámetros formales necesarios */
{
    $puntaje=7-$inten;
    $cadenaLetras=str_split($palabra);
    for($i=0;$i<count($cadenaLetras);$i++){
        if($cadenaLetras[$i]=="A" ||$cadenaLetras[$i]=="E" ||$cadenaLetras[$i]=="I" ||$cadenaLetras[$i]=="O" ||$cadenaLetras[$i]=="U" ){
            $puntaje=$puntaje+1;
        }else{
            if($cadenaLetras[$i]<="M"){
                $puntaje=$puntaje+2;
